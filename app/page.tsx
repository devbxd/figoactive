...
export default async function HomePage() {
  const [products, categories, featured, content] = await Promise.all([
    getProducts(),
    getCategories(),
  ...
  const philosophyImage = content.philosophyImageUrl || editorial?.images[0];
    ...
    <main className="overflow-x-hidden">
      <section className="relative flex min-h-screen items-end overflow-hidden bg-brand-navy text-white">
        ...
        <div className="relative mx-auto w-full max-w-6xl px-4 pb-20 pt-52 md:px-6 md:pb-28 md:pt-60">
          ...
          <p className="mt-6 max-w-md text-sm leading-relaxed text-white/80 md:text-base">{content.heroSubtext}</p>
      ...
      {philosophyImage && (
        <section className="relative flex min-h-[70vh] items-center justify-center overflow-hidden bg-brand-navy text-center text-white">
          {/* Plain <img>, not next/image: the admin can paste any external image
              URL here, and next/image would throw on a host missing from
              next.config.ts's remotePatterns allowlist. */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={philosophyImage} alt="" className="absolute inset-0 h-full w-full object-cover opacity-50" />
      ...
      <section className="mx-auto max-w-6xl px-4 py-20 text-center md:px-6">
        <p className="font-heading text-xs font-semibold uppercase tracking-[0.3em] text-neutral-400">
