...
export async function createProduct(formData: FormData) {
  ...
  const { data: product, error } = await supabase
    .from("products")
    .insert({ name, slug, category_id: categoryId, description, price, compare_at_price: compareAtPrice })
    .select("id, slug")
    .single();
...
export async function updateProduct(productId: string, formData: FormData) {
  ...
  await supabase
    .from("products")
    .update({ name, category_id: categoryId, description, price, compare_at_price: compareAtPrice, is_active: isActive })
    .eq("id", productId);
