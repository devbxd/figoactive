...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex h-[4.5rem] items-center justify-around border-t border-white/10 bg-brand-navy text-white md:hidden">
      <Link href="/" className="flex flex-col items-center gap-1 text-white/70 hover:text-white">
        <HomeIcon />
        <span className="text-[11px] leading-none">Home</span>
      </Link>
      <Link href="/shop" className="flex flex-col items-center gap-1 text-white/70 hover:text-white">
        <ShopIcon />
        <span className="text-[11px] leading-none">Shop</span>
      </Link>
      ...
      <a
        ...
        className="-mt-6 flex h-14 w-14 items-center justify-center rounded-full bg-brand-mint text-brand-navy shadow-lg"
        aria-label="WhatsApp"
      >
        <WhatsAppIcon className="h-6 w-6" />
      </a>

      <Link href="/cart" className="relative flex flex-col items-center gap-1 text-white/70 hover:text-white">
        <CartIcon />
        {count > 0 && (
          <span className="absolute -right-2 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-brand-mint text-[10px] font-bold text-brand-navy">
            {count}
          </span>
        )}
        <span className="text-[11px] leading-none">Cart</span>
      </Link>
      <Link href="/wishlist" className="flex flex-col items-center gap-1 text-white/70 hover:text-white">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="h-6 w-6">
          <path
